...
> ⚠️ **历史归档 (Historical Archive)** — 本文档是项目**早期设计阶段**的产物，描述的是内置 LLM 的 ReAct Agent 循环架构。
>
> 当前架构已演变为**纯执行器 + 安全网关**模式：
> - LLM 推理已**外部化**（由 Claude Code / Cursor / Python 脚本驱动）
> - 通信已统一为**单进程 Native Host**（WebSocket + MCP SSE + HTTP 三合一）
> - 详见当前设计方案：**[design.md](design.md)**
>
> 本文档仅作为设计演进的历史参考保留。其中的伪代码、技术决策和分期规划可能与当前实现有显著差异。
>
> ---
